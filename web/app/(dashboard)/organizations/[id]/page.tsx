...
import { mapBedrijvenRow, mapObjectenRow, mapInspecteurRow, escapeForSelector } from '@/lib/appsheet/mappers';
...
export default async function OrgDetailPage({ params }: Props) {
  ...
  if (source === 'appsheet') {
    return <AppsheetOrgDetail bedrijfId={params.id} />;
  }
...
// AppSheet-sourced organisation detail. Bedrijven has no address/lat-long
// columns (see mapBedrijvenRow) and Objecten has no zone/session/label
// rollups the way building_summary does — those stay an explicit notice
// instead of silently showing zero, same pattern as AppsheetBuildingDetail
// in buildings/[id]/page.tsx.
async function AppsheetOrgDetail({ bedrijfId }: { bedrijfId: string }) {
  const idFilter = escapeForSelector(bedrijfId);
  const [bedrijvenResult, inspecteursResult, objectenResult] = await Promise.all([
    appsheetFind('Bedrijven', `FILTER(Bedrijven, [Bedrijf ID] = "${idFilter}")`),
    appsheetFind('Inspecteurs', `FILTER(Inspecteurs, [Bedrijf ID] = "${idFilter}")`),
    appsheetFind('Objecten', `FILTER(Objecten, [Bedrijfs ID] = "${idFilter}")`),
  ]);

  const row = Array.isArray(bedrijvenResult) ? bedrijvenResult[0] : undefined;
  if (!row) notFound();

  const org = mapBedrijvenRow(row);
  ...
  const buildings = (Array.isArray(objectenResult) ? objectenResult : [])
    .map((r: Record<string, unknown>) => mapObjectenRow(r, undefined));
      ...
      <div>
        ...
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">{org.name}</h1>
              <p className="text-sm text-gray-500 mt-0.5">Bedrijf ID {org.id}</p>
            </div>
          </div>
          <div className="flex gap-5 text-sm text-gray-500 shrink-0">
            <span><span className="font-semibold text-gray-900">{users.length}</span> users</span>
